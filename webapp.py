...
import bz2
import pandas as pd
#st.set_page_config(page_title="Prostate Cancer Recommendation", page_icon="fevicon.png", layout="centered", initial_sidebar_state="auto", menu_items=None)
...
if choice=="Home":
    st.markdown(
        """
        <h2 style="color:white">Welcome to Prostate Cancer Recommendation</h2>
        <h1>    </h1>
        <p align="justify">
        ...
        if re.fullmatch(regex, Email):
            ...
            if Email=='a@a.com' and Password=='123':
                ...
                if result:
                    ...
                    texture=st.text_input("Enter texture=")
                    perimeter=st.text_input("Enter perimeter=")
                    area=st.text_input("Enter area=")
                    smoothness=st.text_input("Enter smoothness=")
                    compactness=st.text_input("Enter compactness=")
                    symmetry=st.text_input("Enter symmetry=")
                    fractal_dimension=st.text_input("Enter fractal_dimension=")
                    b2=st.button("Recommand")
                    sfile = bz2.BZ2File('model.pkl', 'r')
                    model=pickle.load(sfile)
                    tdata=[radius,texture,perimeter,area,smoothness,compactness,symmetry,fractal_dimension]
                    import numpy as np
                    tdata=np.float_(tdata)    
                    if b2:
